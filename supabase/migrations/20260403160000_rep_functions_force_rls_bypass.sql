...
-- Dupla camada: SET row_security = off no cabeçalho da função + set_config no corpo.
...
COMMENT ON FUNCTION public.rep_register_punch IS
  'REP-P: NSR + hash. row_security=off + set_config: INSERTs exigem bypass RLS.';

COMMENT ON FUNCTION public.rep_register_punch_secure IS
  'REP-P + antifraude. row_security=off + set_config: INSERTs exigem bypass RLS.';

COMMENT ON FUNCTION public.insert_punch_evidence_for_own_punch IS
  'Evidência do próprio usuário; row_security=off + set_config após validação.';
