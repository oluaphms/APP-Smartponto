...
BEGIN
  ...
  SELECT 
    ...
    ws.break_start_time,
    ws.break_end_time,
    COALESCE(ws.tolerance_minutes, 0)
  INTO 
    v_shift_start, 
    v_shift_end,
    v_break_start,
  ...
  v_entrada_mins := EXTRACT(HOUR FROM v_local_ts)::int * 60 + EXTRACT(MINUTE FROM v_local_ts)::int;
  
  -- Determinar tipo baseado na sequência e horários da escala
  CASE v_existing_count
    WHEN 0 THEN
      -- Primeira batida = entrada
      v_tipo_tr := 'entrada';
      v_start_mins := EXTRACT(HOUR FROM v_shift_start)::int * 60 + EXTRACT(MINUTE FROM v_shift_start)::int;
      v_is_late := v_entrada_mins > (v_start_mins + v_tol);
      
    WHEN 1 THEN
      -- Segunda batida: verificar se está no horário do intervalo
      IF v_break_start IS NOT NULL AND v_break_end IS NOT NULL THEN
        v_start_mins := EXTRACT(HOUR FROM v_break_start)::int * 60 + EXTRACT(MINUTE FROM v_break_start)::int;
        -- Se está próximo do início do intervalo, é saída para intervalo
        IF v_entrada_mins >= (v_start_mins - 30) AND v_entrada_mins <= (v_start_mins + 60) THEN
          v_tipo_tr := 'saída';
        ELSE
          v_tipo_tr := 'saída';  -- saída antecipada ou sem intervalo
        END IF;
      ELSE
        -- Sem intervalo configurado, segunda batida é saída
        v_tipo_tr := 'saída';
      END IF;
      
    WHEN 2 THEN
      -- Terceira batida: entrada (retorno do intervalo) ou entrada após saída parcial
      IF v_break_end IS NOT NULL THEN
        v_start_mins := EXTRACT(HOUR FROM v_break_end)::int * 60 + EXTRACT(MINUTE FROM v_break_end)::int;
        -- Se está próximo do fim do intervalo, é entrada de retorno
        IF v_entrada_mins >= (v_start_mins - 30) AND v_entrada_mins <= (v_start_mins + 60) THEN
          v_tipo_tr := 'entrada';
        ELSE
          v_tipo_tr := 'entrada';
    ...
    ELSE
      -- Batidas adicionais: alternar baseado na paridade
      IF v_existing_count % 2 = 0 THEN
        v_tipo_tr := 'entrada';
      ELSE
        v_tipo_tr := 'saída';
...
BEGIN
  ...
  -- Buscar usuário
  SELECT u.id::text, u.id::uuid INTO v_user_id, v_user_uuid FROM public.users u
